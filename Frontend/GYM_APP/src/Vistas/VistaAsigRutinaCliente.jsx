...
import FormRegGestRutina from '../Formularios/FormRegAsigRutinaCliente.jsx'; // Asegúrate que esta ruta sea correcta
import "./VistaAsigRutinaCliente.css"
import ExpGrupoRutina from "../Expedientes/ExpRutinas.jsx"; // Cambio Agregado Exp
    ...
    const handleSubmit = async (id) => {
        ...
        try {
            ...
            const response = await axios.post(
                UrlWithApiDG(ENDPOINTS.eliminarRutina),
                { Id_Rutina : id }
            );
    ...
    return (
        <main className="main">
            <header className="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-4">
                ...
                {showForm && <FormRegGestRutina onClose={() => setShowForm(false)} />}
            </header>
            ...
            {selectedUserId !== null && (
                <div className="modal-overlay" onClick={() => setSelectedUserId(null)}>
                    <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                        <ExpGrupoRutina rutinaId={selectedUserId} onClose={() => setSelectedUserId(null)} />
