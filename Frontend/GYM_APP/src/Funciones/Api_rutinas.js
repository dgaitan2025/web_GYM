...
async function getGrupoMuscular() {
  const response = await fetch(UrlWithApiDG(ENDPOINTS.listarRutinas));
...
export const insertarRutina = async (clienteData) => {
   
  try {
    
    const response = await fetch(UrlWithApiDG(ENDPOINTS.insetarRutina), {
      method: 'POST',
      headers: {
